...
import { Plus, Trash, Edit, Save, Film, Briefcase, Cpu, Settings, Eye, ChevronRight, ChevronUp, ChevronDown, User, X, Menu, Upload, Youtube, Image, Loader2, Link2, Calendar, ChevronLeft } from 'lucide-react';
...
const Dashboard = () => {
    ...
    return (
        <div className="min-h-screen text-white relative flex flex-col bg-[#050505]">
            {/* Mobile Header */}
            <header className="lg:hidden flex items-center justify-between p-6 bg-black/40 backdrop-blur-xl border-b border-white/5 sticky top-0 z-[100]">
                <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-primary rounded-full shadow-[0_0_10px_rgba(255,59,48,0.8)]" />
                    <span className="text-xs font-black tracking-widest uppercase">Console</span>
                </div>
                <button
            ...
            <div className="flex-1 flex flex-col lg:flex-row relative z-10 overflow-hidden">
                {/* Sidebar Navigation */}
                <aside className={`
                    fixed inset-y-0 left-0 z-50 w-72 bg-black/90 backdrop-blur-[50px] border-r border-white/5 p-8 flex flex-col transition-transform duration-300 transform
                    lg:relative lg:translate-x-0 lg:bg-white/[0.02] lg:z-0 lg:h-screen lg:sticky lg:top-0
                ...
                `}>
                    <div className="flex items-center gap-4 mb-12">
                        ...
                        <div>
                            <p className="text-[10px] text-gray-500 uppercase tracking-[0.2em] font-bold">Admin Portal</p>
                            <p className="text-lg font-bold text-white tracking-tight">{user?.user_metadata?.full_name?.split(' ')[0] || 'Baraa'}</p>
                        </div>
                    </div>

                    <nav className="flex-1 space-y-2">
                        <div className="text-[10px] text-gray-600 uppercase tracking-[0.2em] font-bold mb-4 ml-2">Management</div>
                        <button
                            onClick={() => { setActiveTab('gallery'); setIsSidebarOpen(false); }}
                            className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'gallery' ? 'bg-primary text-white shadow-lg shadow-primary/20 scale-[1.02]' : 'text-gray-400 hover:bg-white/5'}`}
                        >
                            <Film size={20} /> <span className="text-sm font-bold tracking-wide">Gallery Master</span>
                        </button>
                        <button
                            onClick={() => { setActiveTab('resume'); setIsSidebarOpen(false); }}
                            className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'resume' ? 'bg-primary text-white shadow-lg shadow-primary/20 scale-[1.02]' : 'text-gray-400 hover:bg-white/5'}`}
                        >
                            <Briefcase size={20} /> <span className="text-sm font-bold tracking-wide">Career Hub</span>
                        </button>
                        <button
                            onClick={() => { setActiveTab('skills'); setIsSidebarOpen(false); }}
                            className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'skills' ? 'bg-primary text-white shadow-lg shadow-primary/20 scale-[1.02]' : 'text-gray-400 hover:bg-white/5'}`}
                        >
                            <Cpu size={20} /> <span className="text-sm font-bold tracking-wide">Tech Arsenal</span>
                        </button>
                        <div className="pt-4 border-t border-white/5 mt-4">
                            <button
                                onClick={() => { setActiveTab('settings'); setIsSidebarOpen(false); }}
                                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'settings' ? 'bg-primary text-white shadow-lg shadow-primary/20 scale-[1.02]' : 'text-gray-400 hover:bg-white/5'}`}
                            >
                                <Settings size={20} /> <span className="text-sm font-bold tracking-wide">Global Systems</span>
                            </button>
                        </div>
                    </nav>

                    <div className="mt-auto space-y-4">
                        <Link to="/" className="flex items-center gap-3 px-4 py-3 text-sm text-gray-400 hover:text-white transition-all bg-white/5 rounded-2xl border border-white/5 group">
                            <Eye size={18} className="group-hover:text-primary" />
                            <span className="font-bold uppercase tracking-widest text-[10px]">Preview Site</span>
                        </Link>
                        <div className="p-4 bg-gradient-to-br from-primary/20 to-transparent rounded-2xl border border-primary/10">
                            <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">System Status</p>
                            <div className="flex items-center gap-2">
                                <div className="w-1.5 h-1.5 bg-primary rounded-full animate-ping" />
                                <span className="text-[10px] text-gray-300 font-medium">Baraa-CMS v2.4 Online</span>
                            </div>
                        </div>
                    </div>
                </aside>
                ...
                {/* Main Content Area */}
                <main className="flex-1 p-4 sm:p-8 lg:p-12 overflow-y-auto max-h-[calc(100vh-80px)] lg:max-h-screen custom-scrollbar">
                    {/* Perspective Header */}
                    ...
                    {activeTab === 'gallery' && (
                        <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
                            ...
                            <div className="xl:col-span-3 space-y-6">
                                <section className="bg-white/[0.03] backdrop-blur-[40px] border border-white/10 rounded-[1.5rem] sm:rounded-[2rem] p-6 sm:p-10 relative overflow-hidden group shadow-2xl">
                    ...
                    {isOpen && (
                        <>
                            <motion.div
                                ...
                                className="absolute left-0 w-[320px] top-[calc(100%+16px)] p-6 bg-white/[0.01] backdrop-blur-[80px] border border-white/10 rounded-[2.5rem] shadow-[0_50px_120px_rgba(0,0,0,0.9)] z-[100] overflow-hidden"
                            >
...
const ExperienceEntry = ({ job, onUpdate, onDelete, onReorder, isFirst, isLast, prevOrder, nextOrder }) => {
    ...
    return (
        ...
        >
            {/* Timeline track */}
            <div className="absolute left-[34px] top-12 bottom-[-48px] w-px bg-gradient-to-b from-primary via-primary/10 to-transparent z-0" />

            <div className="relative z-10 bg-white/[0.03] backdrop-blur-3xl border border-white/5 p-10 rounded-[3rem] hover:bg-white/[0.05] hover:border-primary/30 transition-all duration-500 shadow-2xl overflow-hidden">
                {/* Ambient Glow */}
                    ...
                    <div className="flex-1 space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-3">
                                ...
                                <input
                                    ...
                                    className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-xl font-black text-white focus:border-primary/50 outline-none transition-all hover:bg-black/60 placeholder:text-gray-800 uppercase tracking-tighter"
                            ...
                            <div className="space-y-3">
                                ...
                                <input
                                    ...
                                    className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-sm font-mono text-primary focus:border-primary/50 outline-none transition-all hover:bg-black/60 placeholder:text-gray-800 uppercase tracking-widest"
                        ...
                        <div className="space-y-3">
                            ...
                            <input
                                ...
                                className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-2xl font-black text-white focus:border-primary/50 outline-none transition-all hover:bg-black/60 placeholder:text-gray-800 uppercase tracking-tighter"
...
const SkillGroupEntry = ({ group, onUpdate, onDelete, onReorder, isFirst, isLast, prevOrder, nextOrder }) => {
    ...
    return (
        <motion.div
            layout
            className="group relative bg-white/[0.03] backdrop-blur-3xl border border-white/5 p-10 rounded-[3rem] hover:bg-white/[0.05] hover:border-blue-500/30 transition-all duration-500 shadow-2xl overflow-hidden flex flex-col h-full"
        >
            {/* Ambient Bloom */}
            ...
            <div className="flex justify-between items-start mb-10 relative z-10">
                <div className="flex-1 mr-8">
                    ...
                    <input
                        ...
                        className="w-full bg-transparent border-none p-0 text-3xl font-black text-white hover:text-blue-400 transition-colors outline-none uppercase tracking-tighter"
